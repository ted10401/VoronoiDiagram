...
{
    public class BreathFirstSpanningTree : BaseSpanningTree
    {
        private Queue<Vector3> m_processQueue;
        private Dictionary<Vector3, List<STSegment>> m_neighborSegments;

        public BreathFirstSpanningTree(STEdge[] stEdges) : base(stEdges)
        {
        }
        ...
        {
            m_processQueue = new Queue<Vector3>();

            int cacheRootA;
            int cacheRootB;
            STSegment cacheSegment;
            m_processQueue.Enqueue(InputPoints[Random.Range(0, InputPoints.Count)]);

            while (m_processQueue.Count != 0)
            {
                cacheSegment = GetRandomSegment(m_processQueue.Peek());
